...
2026-09-08 기준 Phase 1–3은 `fe642c3`, `94c615a`, `1973cba`로 구현되었다. Phase 4 다중 심볼 구독은 `45d9102`, Phase 5 출력 연결과 통계는 `a1db0bf`로 구현되었으며, Phase 6 TUI와 통합 검증도 완료했다. 아래 초기 계획은 해당 구현의 기준으로 보존하고, 수집 심볼·실행 모드·출력 연결에 대한 판단은 문서 후반의 Phase 4–6 계약을 우선한다.
...
Phase 7–10 완료 시 Python 3.12.13 새 가상환경에 고정 의존성 8개를 설치하고 `pip check`와 기본 자동화 테스트 65개를 통과했다. 별도의 실제 MongoDB 통합 테스트 1개도 MongoDB 8.0.28에서 통과했다. PyMongo 4.18.0 환경의 실제 Binance 10개 심볼 수집은 약 66초 동안 거래 16,329건과 구독 응답 1건, 총 16,330개 문서를 저장했으며 가격·수량 문자열 타입과 모든 심볼의 존재를 확인했다. 검증에 사용한 고유 컬렉션은 확인 후 삭제했고 운영 컬렉션은 변경하지 않았다.
