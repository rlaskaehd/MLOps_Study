...
라이브 검증은 다음 순서로 진행한다. 2026-09-17에 20초 실제 수신과 12초 실제 MongoDB sink 검증을 완료했으며, 아래 30분·26시간 검증은 후속 장시간 검증으로 남아 있다.
...
확장 모드 적용 전에는 v2 문서 구조를 소비자가 읽을 수 있는지 확인한다. 롤백은 확장 실행을 중단하고 기존 profile로 재시작하는 방식이며 신규 컬렉션을 삭제하지 않는다. 26시간 검증까지 끝나면 Phase 16의 장시간 검증을 완료로 표시한다.
