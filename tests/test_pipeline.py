...
class EventDispatcherTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_forwards_all_messages_once_in_order_with_values_preserved(self) -> None:
        output = MemoryOutput()
        ...
        self.assertEqual(output.calls, ["open", "write", "write", "write", "write", "close"])
        self.assertEqual(
            output.events,
            [
                {"result": None, "id": 1},
